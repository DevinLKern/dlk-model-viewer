...
use crate::{Input, Result, WORLD_FORWARDS, WORLD_RIGHT, WORLD_UP, result::Error};
...
pub enum Command {
    ...
    ObjectRotation,
    Rotate,
    // RotateForwardPositive,
    // RotateForwardNegative,
    // RotateRightPositive,
    // RotateRightNegative,
    // RotateUpPositive,
    // RotateUpNegative,
...
impl Settings {
    pub fn new(settings_path: &std::path::Path, _args: &[String]) -> Result<Self> {
        use yaml_rust2::Yaml;
            ...
            let bindings_yaml = all.get(&Yaml::String(String::from("bindings")))
            ...
            for binding_yaml in bindings_yaml.iter().filter_map(|b| b.as_hash()) {
                let command = binding_yaml.get(&Yaml::String(String::from("command")))
                    .unwrap()
                    .as_str()
                    .unwrap();
                ...
                let input = binding_yaml.get(&Yaml::String(String::from("input")))
                    .ok_or(Error::ConfigFileInvalid("input not found"))?
                    .as_str()
                    .ok_or(Error::ConfigFileInvalid("expected input value to be a string"))?;
                let input = Input::from_str(input)
                    .ok_or(Error::ConfigFileInvalid("invalid input value"))?;
                
                let event = binding_yaml.get(&Yaml::String(String::from("event")))
                    .ok_or(Error::ConfigFileInvalid("event not found"))?
                    .as_str()
                    .ok_or(Error::ConfigFileInvalid("expected event value to be a string"))?;
                let event = Event::from_str(event)
                    .ok_or(Error::ConfigFileInvalid("invalid event value"))?;
                ...
                bindings.push(Binding{ command, input, event, requirement });
            }

            let options = all.get(&Yaml::String(String::from("options")))
                .ok_or(Error::ConfigFileInvalid("options not found"))?
                .as_hash()
                .ok_or(Error::ConfigFileInvalid("expected options to be a hash"))?;

            let fov_y = options.get(&Yaml::String(String::from("fov_y")))
                .ok_or(Error::ConfigFileInvalid("fov_y not found"))?
                .as_f64()
                .ok_or(Error::ConfigFileInvalid("expected fov_y to be a float"))?;

            let mouse_sensitivity = options.get(&Yaml::String(String::from("mouse_sensitivity")))
                .ok_or(Error::ConfigFileInvalid("mouse sensitivity not found"))?
                .as_f64()
                .ok_or(Error::ConfigFileInvalid("expected mouse_sensitivity to be a float"))?;

            let derive_normals = options.get(&Yaml::String(String::from("derive_normals")))
                .ok_or(Error::ConfigFileInvalid("derive_normals not found"))?
                .as_bool()
                .ok_or(Error::ConfigFileInvalid("expected derive normals to be true"))?;
            ...
            let model_up = options.get(&Yaml::String(String::from("model_up")))
                .ok_or(Error::ConfigFileInvalid("model_up value not found"))?
                .as_str()
                .ok_or(Error::ConfigFileInvalid("expected mode_up to be a str"))?;
            let model_up = str_to_vec(model_up)
                .ok_or(Error::ConfigFileInvalid("invalid model_up value"))?;

            let model_right = options.get(&Yaml::String(String::from("model_right")))
                .ok_or(Error::ConfigFileInvalid("model_right value not found"))?
                .as_str()
                .ok_or(Error::ConfigFileInvalid("expected mode_right to be a str"))?;
            let model_right = str_to_vec(model_right)
                .ok_or(Error::ConfigFileInvalid("invalid model_up value"))?;

            let model_forward = options.get(&Yaml::String(String::from("model_forward")))
                .ok_or(Error::ConfigFileInvalid("model_forward value not found"))?
                .as_str()
                .ok_or(Error::ConfigFileInvalid("expected mode_forward to be a str"))?;
            let model_forward = str_to_vec(model_forward)
                .ok_or(Error::ConfigFileInvalid("invalid model_up value"))?;
